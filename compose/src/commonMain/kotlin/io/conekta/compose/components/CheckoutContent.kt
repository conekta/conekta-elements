...
private fun createTokenizerService(config: CheckoutConfig): TokenizerApiService {
    val tokenizerConfig =
        TokenizerConfig(
            publicKey = config.publicKey,
            baseUrl = config.tokenizerBaseUrl,
        ).let { baseConfig ->
            config.tokenizerRsaPublicKey
                ?.takeUnless { it.isBlank() }
                ?.let { baseConfig.copy(rsaPublicKey = it) }
                ?: baseConfig
        }
    return TokenizerApiService(
    ...
    try {
        ...
        checkoutService
            ...
            .onFailure { e ->
                ...
                onError(error)
                onSubmitError(error.errorMessage)
            }
    } catch (e: CheckoutApiException) {
        onError(e.checkoutError)
        onSubmitError(e.checkoutError.errorMessage)
    } catch (e: Exception) {
